...
clean_wind_data <- as.numeric(clean_wind_data)
clean_wind_data
ts_clean_wind <- ts(clean_wind_data,frequency=12) 

decompose_clean_wind=decompose(ts_clean_wind,"additive")

plot(decompose_clean_wind)

#Ex2: Store the three component in separate vector

decompose_trend = decompose_clean_wind$trend
decompose_seas = decompose_clean_wind$seasonal
decompose_rand = decompose_clean_wind$random

#Ex3: Create one data frame with all three vector and the Date columns from wind_data

df_wind_decomposed <- data.frame("Date" = full_wind_data$Date, "Trend" = decompose_trend, "Seasonal" = decompose_seas, "Random" = decompose_rand)
