...
dfForecast <- data.frame("Date" = electricity_price_processed$Month[(nobs-n_for+1):nobs], observed, MEAN_seas$mean,SNAIVE_seas$mean,SARIMA_for$mean)

#Use ggplot to show observed and forecast on a single plot.

ggplot(dfForecast, aes(x=Date)) +
    ylab(paste0("Electricity Price") +
    geom_line(aes(y=observed), color = "black") +
    geom_line(aes(y=MEAN_seas.mean), color = "blue") +
    geom_line(aes(y=SNAIVE_seas.mean), color = "orange") +
    geom_line(aes(y=SARIMA_seas.mean), color = "green")
         )
